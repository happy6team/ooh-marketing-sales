    ...
    try:
        with st.spinner(f"{brand} 제안서 생성 중..."):
            ...
            with st.expander("제안서 생성 로그", expanded=False):
                st.subheader("표준 출력:")
                st.code(result.stdout[:3000] + "\n... (생략됨)" if len(result.stdout) > 3000 else result.stdout)
                if result.stderr:
                    st.subheader("오류 출력:")
                    st.code(result.stderr[:3000] + "\n... (생략됨)" if len(result.stderr) > 3000 else result.stderr)
            ...
            # --- 성공 처리 ---
            if result.returncode == 0 and json_result and json_result.get("success"):
            ...
            # --- 실패 처리 ---
            st.error(f"❌ JSON 결과가 없거나 실패했습니다: {json_result.get('error') if json_result else 'None'}")
